...
class NationalCodeValidator(RegexValidator):
    regex = '^(\d{10})$'
    message = 'National Code must be a valid 10 digits'
...
class SalaryValidator(RegexValidator):
    regex = '^[0-9]+$'
    message = _('Enter a Valid salary')
    code = 'Invalid_salary'


validate_salary = SalaryValidator()
...
validate_national_code = NationalCodeValidator()
